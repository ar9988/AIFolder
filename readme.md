...
* limitedParallelism을 활용한 파일 스캔 병렬성 최적화
* 300GB 이상 실기기 환경에서 약 100초 내 전체 파일 인덱싱
...
온디바이스 임베딩 모델을 활용하여 서버 없이 태그 추천과 의미 기반 검색을 제공합니다.
...
* Partial Hash 기반 해시 계산으로 디스크 읽기 비용 감소
* BFS + limitedParallelism(4) 조합을 벤치마크를 통해 최종 선택

결과

* 300GB 이상 실기기 환경에서 약 2분 내 전체 파일 스캔 완료
